...
class Controller:
    ...
    estante_db = shelve.open("estante.db", writeback=True)
    
    def salvar_estante():
        Controller.estante_db['estante'] = Controller.estante.get_lista_livros()
        Controller.estante_db.close()
        
    def carregar_estante():
        if 'estante' in Controller.estante_db:
            Controller.estante.livros_comprados = Controller.estante_db['estante']
    ...
    def pesquisar_livros_por_titulo(titulo):
        ...
        if livros:
            ...
            View.View.imprimir(f"ERRO, não existem livros com o titulo {titulo}")
    ...
    def get_status(id):
        ...
        if livro:
            ...
            View.View.imprimir(f"ERRO, não existe livro cadastrado com o id {id}")
            
    def get_percentual_leitura(id):
        livro = Controller.estante.get_livro_por_id(id)
        if livro:
            percentual_leitura = livro.get_percentual_leitura()
            View.View.imprimir(f"Percentual de leitura do livro '{id}': {percentual_leitura}%")
        else:
            View.View.imprimir(f"ERRO, não existe livro cadastrado com o id {id}")   
    ...
    def get_quant_paginas_restantes(id):
        ...
        if livro:
            ...
            View.View.imprimir(f"Quantidade de páginas restantes do livro '{id}': {quant_paginas_restantes}")
        else:
            View.View.imprimir(f"ERRO, não existe livro cadastrado com o id {id}")
